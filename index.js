...
class MiFlora extends EventEmitter {
  ...
  parseData(peripheral, data) {
    ...
    let devData = {temperature: data.readUInt16LE(0) / 10, lux: data.readUInt32LE(3), moisture: data.readUInt16BE(6), fertility:data.readUInt16LE(8)};
                   
    //this.emit('data', deviceData);
    this.emit('data', devData);
  }
