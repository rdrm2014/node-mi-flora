...
class MiFlora extends EventEmitter {
  ...
  parseData(peripheral, data) {
    ...
    let devData = {deviceId: peripheral.id, temperature: data.readUInt16LE(0) / 10, lux: data.readUInt32LE(3), moisture: data.readUInt16BE(6), fertility: data.readUInt16LE(8)};
    ...
    this.emit('data', {deviceId: devData.deviceId, temperature: devData.temperature, lux: devData.lux,moisture: devData.moisture,fertility: devData.fertility });
  }
  ...
  parseFirmwareData(peripheral, data) {
    ...
    this.emit('firmware', { deviceId: firmware.deviceId,batteryLevel: firmware.batteryLevel,firmwareVersion: firmware.firmwareVersion });
...
flora.on('firmware', function (data) {
  flora.on('data', function (dataDevice) {
    console.log('firmware123', data);
    console.log('data123', dataDevice);
    message = JSON.parse(data).concat(JSON.parse(dataDevice));
    client.publish(topic, new Buffer(message), {}, function() {    
      console.log('done!');
    });
  });
});